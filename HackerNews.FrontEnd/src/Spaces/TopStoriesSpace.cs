...
{
    public class TopStoriesSpace : IComponent
    {
        private IComponent _content;

        public TopStoriesSpace(Parameters state)
        {
            _content = HubStack("Top Stories", Routes.TopStories, DefaultRoutes.Home)
                            .Section(Defer(async () => 
                            {
                                ...
                                return SearchArea().OnSearch(sr => sr.SetBeforeTypesFacet(N.Story.Type)
                                                                 .WithTargetQuery(Mosaik.API.Query.StartAt(N.TopStories.Type, currentLatestNode.GetString(N.Property.ValueString)).Out(N.Story.Type).Take(500))
