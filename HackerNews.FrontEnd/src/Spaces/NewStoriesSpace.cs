...
{
    public class NewStoriesSpace : IComponent
    {
        private IComponent _content;

        public NewStoriesSpace(Parameters state)
        {
            _content = HubStack("New Stories", Routes.NewStories, DefaultRoutes.Home)
                            .Section(Defer(async () => 
                            {
                                ...
                                return SearchArea().OnSearch(sr => sr.SetBeforeTypesFacet(N.Story.Type)
                                                                     .WithTargetQuery(Mosaik.API.Query.StartAt(N.NewStories.Type, currentLatestNode.GetString(N.Property.ValueString)).Out(N.Story.Type).Take(500))
