...
{
    public class PropertyRenderer : INodeRenderer
    {
        public string NodeType    => N.Property.Type;
        ...
        private IComponent CreateView(Node node, Parameters state)
        {
            return VStack().S().ScrollY().Children(
                        Label().WS().Inline().AutoWidth().SetContent(TextBlock(node.GetString(N.Property.Name)))
               );
